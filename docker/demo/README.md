...
环境变量：`DEMO_PORT` / `SEALOS_KUBECONFIG_PATH` / `DSH_IMAGE`（默认 eftik-dsh-cloud:0.6.0）/ `DSH_CPU` / `DSH_MEM`，产品模式见下文
...
## 产品模式（网关 v0.6 / 镜像 0.6.0）

`.env` 里开启 `GW_PRODUCT_MODE=1` 后，demo 充当平台（kitsume 后端角色）：

- 创建的工作台为 0.6.0 镜像，注入 `GW_PRODUCT_MODE` / `GW_ADMIN_TOKEN` / `GW_PRESET_*`
